...
if [ ! -d "/var/www/pterodactyl_backup" ]; then
  echo "Direktori pterodactyl_backup tidak ada, silakan hubungi Dev"
  exit 1
else
...
if [ -f "$BLUEPRINT_PATH" ]; then
    show_progress "Blueprint sudah terpasang. Melanjutkan instalasi tema."
    cd /root
    cd /var/www
    ...
    show_progress "Blueprint belum terpasang. Memulai proses instalasi Blueprint dan dependensi."
    ...
    sudo apt install -y zip unzip nodejs git curl wget
